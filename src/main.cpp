...
int main()
{
  ...
  PID pid_steering;
  PID pid_throttle;
  pid_steering.Init(0.21, 0.004, 3.01);
  pid_throttle.Init(0.2, 0.000, 0.0);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          steer = true;
          steer_value = pid_steering.TotalError(speed, steer);

          pid_throttle.UpdateError(fabs(angle));
          steer = false;
          throttle_value = 0.5; //- pid_throttle.TotalError(speed, steer);
